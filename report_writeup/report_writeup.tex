...
\vspace*{-2mm}
\caption{Coefficients, standard errors, and p-values from the regression of felonies on temperature.}
\label{tab:lm1}   
\vspace*{-3mm}
\end{table}
...
We next incorporate additional covariates, performing a multiple linear regression of felonies on the variables shown in the regression summary in Table \ref{tab:lm4}. \todo{update caption}
...
We re-fit the regression from Section \ref{sec:modelFeloniesMultipleRegression}, but now excluding the 17 potentially problematic observations. The regression summary is shown in Table \ref{tab:lm5}. \todo{update the caption}
...
We now perform a multiple linear regression of \texttt{felonies_diff} on the variables shown in the regression summary in Table \ref{tab:lmd2}. \todo{update the caption}
...
To simplify the model, we now iteratively remove the non-significant variables from the regression, and end up with a model in which only \texttt{temp_jump} and \texttt{day_of_week} are remaining. The regression summary is shown in Table \ref{tab:lmd6}. \todo{update the caption}
